...
BINARY_NAME := tailnet
PACKAGE := github.com/sudosu404/gnx-cli
...
.PHONY: dev_image
dev_image:
	docker build --build-arg UID=$(shell id -u) --build-arg GID=$(shell id -g) -f dev/Dockerfile.dev -t sudosu404/tailnet:latest .
...
.PHONY: docker_image
docker_image:
	docker buildx build  -t "sudosu404/tailnet:latest" .
